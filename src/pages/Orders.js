    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            if (response.data.status === "success") {
                ...
                console.log("Updating order status to Completed for Order ID:", selectedOrderID);
                const updateResponse = await axios.put(`http://localhost:5000/api/orders/${selectedOrderID}`, { Status: "Completed" }, {
                    headers: { Authorization: `Bearer ${token}` }
                });
    
                console.log("✅ Update Order Response:", updateResponse.data);
